...
export function render() {
  ...
  qtyInput.addEventListener('input', () => {
    ...
    // Mientras el campo esté vacío no forzamos valor; usamos 1 sólo para el cálculo.
    S.calcQty = raw === '' ? 1 : Math.min(999, Math.max(1, parseInt(raw, 10) || 1));
    refreshResult();
  });
  // Al salir del campo, normalizamos lo mostrado (p.ej. vacío → 1).
  qtyInput.addEventListener('blur', () => {
    qtyInput.value = S.calcQty;
  });
