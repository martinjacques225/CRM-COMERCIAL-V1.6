const DB_NAME = 'AgendaComercialDB'; // mismo nombre para preservar datos existentes
const DB_VERSION = 2;
let _db = null;
...
export const appointments = {
  add(data) {
    ...
    return wrap(tx('appointments', 'readwrite').add({
  ...
  update(data) {
    ...
    return wrap(tx('appointments', 'readwrite').put(data));
  },
...
export const leads = {
  add(data) {
    const now = new Date().toISOString();
    return wrap(tx('leads', 'readwrite').add({
  ...
  update(data) {
    data.fechaActualizacion = new Date().toISOString();
    return wrap(tx('leads', 'readwrite').put(data));
  },
...
export const calls = {
  add(data) {
    const now = new Date().toISOString();
    return wrap(tx('calls', 'readwrite').add({
      ...data,
      fecha: data.fecha || now.slice(0, 10),
      hora: data.hora || now.slice(11, 16),
      resultado: data.resultado || 'Iniciada',
      timestamp: now
    }));
  },
...
export const sales = {
  add(data) {
    const now = new Date().toISOString();
    return wrap(tx('sales', 'readwrite').add({
      ...data, fecha: data.fecha || now.slice(0, 10), timestamp: now
    }));
  },
